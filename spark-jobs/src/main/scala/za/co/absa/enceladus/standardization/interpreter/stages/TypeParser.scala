...
object TypeParser {
  ...
  private final case class BinaryParser(field: BinaryTypeStructField,
      ...
      origType match {
        ...
        case StringType =>
          ...
          field.normalizedEncoding match {
            case Some(MetadataValues.Encoding.Base64) => lit(customUnbase64Udf(column))
    ...
    def customUnbase64Udf: UserDefinedFunction =
      udf((encoded: String) => field.stringToTyped(encoded) match {
        case Failure(_) => null // scalastyle:ignore null deliberate to induce Std errCol population
